...
class Login extends Component {
  ...
  render() {
    return (
    	<div id="login">
        <Seo data={{ title: this.context.t('loginTitle'), description: this.context.t('loginDescription'), siteName: this.context.t('siteName') }} />
        <NavigationBar data={{ title: <h1>{this.context.t('loginTitle')}</h1>, subTitle: <h2>{this.context.t('loginDescription')}</h2> }} />
	      <div className="alert alert-info" role="alert">
          <p><Link to="/register">{this.context.t('haveNotAccount')}</Link></p>
          <p><Link to="/forgot-account">{this.context.t('forgotPassword')}</Link></p>
        </div>
        <div className="row">
